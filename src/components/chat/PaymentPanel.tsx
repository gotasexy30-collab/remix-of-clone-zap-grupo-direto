...
export const PaymentPanel: React.FC<PaymentPanelProps> = ({ userCity, userDDD }) => {
  ...
  const toggleVideoPlay = () => {
    ...
    if (!v) return;
    if (v.paused) { void v.play(); setVideoPlaying(true); }
  ...
  useEffect(() => {
    ...
    void checkPayment();
    pollRef.current = setInterval(() => { void checkPayment(); }, 3000);
  ...
  const handleManualCheck = async () => {
    ...
    try {
      ...
      setNotPaidMsg('Não foi possível verificar agora. Tente novamente.');
      setTimeout(() => notPaidRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 100);
    } finally {
      ...
      <div className="w-full sm:max-w-[480px] bg-[#0b141a] flex flex-col h-full relative shadow-2xl">
        <div className="bg-[#1f2c34] px-4 py-2 flex items-center justify-between z-10 shrink-0 h-[60px]">
          <div className="flex items-center gap-3 overflow-hidden">
            <div className="text-[#d9dee0]"><svg viewBox="0 0 24 24" height="24" width="24" fill="currentColor"><path d="M12,4l1.4,1.4L7.8,11H20v2H7.8l5.6,5.6L12,20l-8-8L12,4z"></path></svg></div>
            <div className="w-[40px] h-[40px] rounded-full overflow-hidden shrink-0"><img src="https://midia.jdfnu287h7dujn2jndjsifd.com/IMG-20240711-00350743535.webp" className="w-full h-full object-cover" /></div>
              ...
              <div className="p-4 sm:p-6">
                <div className="flex flex-col items-center gap-4">
                  ...
                  {paymentStatus === 'approved' ? (
                    ...
                    <>
                      <button onClick={() => void handleAccessClick()} disabled={pixLoading} className="w-full bg-[#16A349] text-white py-4 rounded-xl font-bold text-lg shadow-lg active:scale-[0.98] transition-all text-center disabled:opacity-60 flex items-center justify-center gap-2">
                    ...
                    <>
                      <div className="w-full bg-gray-50 rounded-xl p-2 flex flex-col items-center">
                        {pix.qr_code_base64 && <img src={pix.qr_code_base64} alt="QR Code PIX" className="w-36 h-36 object-contain" />}
                      ...
                      <div className="w-full">
                        ...
                        <div className="bg-gray-100 rounded-lg p-2 text-[10px] text-gray-700 break-all max-h-20 overflow-y-auto border">{pix.qr_code}</div>

                        <button onClick={() => void handleCopy()} className="w-full mt-2 bg-[#16A349] text-white py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 active:scale-[0.98]">
                          {copied ? <><Check size={18} /> Código Copiado!</> : <><Copy size={18} /> COPIAR CÓDIGO PIX</>}
                        </button>

                        <button onClick={() => void handleManualCheck()} disabled={checkingManual} className="w-full mt-2 bg-white border-2 border-[#16A349] text-[#16A349] py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 active:scale-[0.98] disabled:opacity-60">
                          {checkingManual ? <><Loader2 size={18} className="animate-spin" /> Verificando...</> : <><Check size={18} /> JÁ PAGUEI</>}
                        </button>

                        <button onClick={() => { setShowVideoModal(true); setVideoPlaying(true); setVideoMuted(false); logTrackedEvent('TutorialOpened'); }} className="w-full mt-2 bg-blue-50 border-2 border-blue-400 text-blue-600 py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 active:scale-[0.98]">
